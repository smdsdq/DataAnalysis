...
def preprocess_csv(input_file, pattern_field, is_streamlit=True):
    try:
        if is_streamlit:
            input_file.seek(0)
        ...
        pattern_field_idx = next((i for i, col in enumerate(headers) if col.lower().strip() == pattern_field.lower()), None)
        if pattern_field_idx is None:
            print_function(f"Pattern field '{pattern_field}' not found in headers. Using first column.", is_streamlit, warning=True)
            pattern_field_idx = 0
        ...
        for row in rows[1:]:
            if len(row) > header_count:
                print_function(f"Found row with {len(row)} fields, expected {header_count}. Fixing '{pattern_field}'...", is_streamlit, warning=True)
                # Assume extra fields are in pattern_field; replace commas and quotes
                fixed_row = row[:pattern_field_idx]
                pattern_content = ','.join(row[pattern_field_idx:pattern_field_idx + len(row) - header_count + 1])
                pattern_content = pattern_content.replace(',', '_').replace('"', "'")
                fixed_row.append(pattern_content)
                fixed_row.extend(row[pattern_field_idx + len(row) - header_count + 1:])
...
def read_csv_with_encodings(uploaded_file, pattern_field, is_streamlit=True):
    ...
    for encoding in encodings:
        try:
            ...
            preprocessed_file = preprocess_csv(uploaded_file, pattern_field, is_streamlit)
            try:
                if is_streamlit:
...
def analyze_data(uploaded_file, max_features, n_clusters_base, ngram_max, pattern_field, stat_fields, group_field, is_streamlit=True):
    try:
        ...
        df, used_encoding = read_csv_with_encodings(uploaded_file, pattern_field, is_streamlit)
        if df is None:
...
def run_cli(args):
    ...
    # Use pattern_field for preprocessing
    pattern_field = args.pattern_field if args.pattern_field else 'description'
...
def run_streamlit():
    ...
    if uploaded_file is not None and review_button:
        try:
            # Default pattern_field for preprocessing
            pattern_field = 'description'
            df, _ = read_csv_with_encodings(uploaded_file, pattern_field, is_streamlit=True)
            if df is not None:
                df.columns = [col.lower().strip() for col in df.columns]
