...
def analyze_data(uploaded_file, max_features, n_clusters_base, ngram_max, pattern_field, stat_fields):
    try:
        # Read CSV with case-insensitive header handling
        ...
        return stats, csv_data, output_df, list(analyzed_df.columns), patterns
...
if st.session_state.available_columns:
    ...
    if analyze_button and pattern_field and stat_fields:
        with st.spinner("Analyzing data..."):
            stats, csv_data, output_df, available_columns, patterns = analyze_data(
...
if st.session_state.stats:
    ...
    # Column Statistics (below overview, expandable if lengthy)
    ...
    if stats['patterns_per_group']:
        ...
        for group, patterns in stats['patterns_per_group'].items():
            st.write(f"**{group}**")
            # Sort patterns by incident_count descending
