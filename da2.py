...
def read_csv_with_encodings(uploaded_file):
    ...
    st.error(f"Failed to read CSV with encodings: {', '.join(encodings)}. Check file encoding and ensure fields with double quotes or line breaks are properly quoted (e.g., \"text\"\"text\" or escaped).")
    return None, None
...
if uploaded_file is not None and review_button:
    try:
        ...
        st.error(f"Failed to read CSV: {str(e)}. Ensure fields with double quotes are escaped (e.g., \"\" or \\") and file encoding is compatible (e.g., UTF-8, Latin-1).")
