...
def analyze_data(uploaded_file, max_features, n_clusters_base, ngram_max, pattern_field, stat_fields):
    try:
        # Read CSV with quoting to handle multiline text
        df = pd.read_csv(uploaded_file, quoting=csv.QUOTE_ALL, escapechar='\\')
        ...
        st.error(f"CSV parsing error, possibly due to multiline text or incorrect formatting: {str(e)}. Ensure fields with line breaks are quoted.")
        return None, None, None, None
...
if uploaded_file is not None and review_button:
    try:
        # Read CSV to get columns, handling multiline text
        df = pd.read_csv(uploaded_file, quoting=csv.QUOTE_ALL, escapechar='\\')
        df.columns = [col.lower().strip() for col in df.columns]
        st.session_state.df = df
        st.session_state.available_columns = list(df.columns)
    except Exception as e:
        st.error(f"Failed to read CSV: {str(e)}. Ensure fields with line breaks are quoted.")
        st.session_state.available_columns = []
